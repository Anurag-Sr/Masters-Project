...
def plot(x, y, savefile, date, avg, std, var):
    """
    Plots x and y data 
    ...
    plt.title('Measurement of Dark Current')
    plt.xlabel('Time in minutes')
    plt.ylabel('Measured current in pico amps (pA, 1e-12A)')
    plt.figtext(0.61,0.97,'Data collected on ' + str(date))
    plt.figtext(0,-0.02,'Note: Data collected over 1 hour with intervals of 30s between each measurement')
    plt.figtext(0, -0.06, '120 measurements in total')
...
def textfile(savetext, mean, std, var):
...
def main():
    ...
    savetext = input('enter name of info file to save as .txt ')
    plot(x,y,savename, date, avg, std, var)
    textfile(savetext, avg, std, var)
